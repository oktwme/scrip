...
namespace LeagueSharpCommon
{
    ...
    public static class Dash
    {
        ...
        /// <returns></returns>
        public static DashItem GetDashInfo(this AIBaseClient unit)
        {
        ...
        {
            if (DetectedDashes.ContainsKey(unit.NetworkId) && !unit.HasBuffOfType(BuffType.Knockup) && unit.Path.Length != 0)
            {
        ...
        private static void ObjAiHeroOnOnNewPath(AIBaseClient sender, AIBaseClientNewPathEventArgs args)
        {
            if (sender.IsValid)
            {
