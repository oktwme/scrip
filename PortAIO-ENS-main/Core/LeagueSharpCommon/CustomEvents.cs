...
namespace LeagueSharpCommon
{
    ...
    public static class CustomEvents
    {
        ...
        public class Game
        {
            ...
            {
                foreach (var hq in ObjectManager.Get<HQClient>().Where(hq => hq.IsValid))
                {
                    NexusList.Add(hq);
                }

                if (EnsoulSharp.Game.State == GameState.Running)
                {
                    //Otherwise the .ctor didn't return yet and no callback will occur
                    DelayAction.Add(500, () => { Game_OnGameStart(); });
                }
                else
                {
            ...
            {
                GameEvent.OnGameTick += Game_OnGameUpdate;
        ...
        public class Unit
        {
            ...
            public delegate void OnDashed(AIBaseClient sender, Dash.DashItem args);
            ...
            public delegate void OnLeveledUp(AIBaseClient sender, OnLevelUpEventArgs args);
            ...
            public delegate void OnLeveledUpSpell(AIBaseClient sender, OnLevelUpSpellEventArgs args);
            ...
            /// <param name="sender">The sender.</param>
            /// <param name="args">The arguments.</param>
            public static void TriggerOnDash(AIBaseClient sender, Dash.DashItem args)
            {
