...
namespace HikiCarry.Core.Plugins
{
    ...
    internal class DashInterrupter
    {
        ...
        public DashInterrupter(Spell blockspell, int range)
        {
            ...
            Range = range;
            Initialize();
            Game.Print("Dash Interrupter Loaded");
            AIBaseClient.OnDoCast += DashProcessSpellCast;
        }

        private void DashProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsEnemy && sender is AIHeroClient && !sender.Spellbook.IsAutoAttack && Utilities.Utilities.Enabled("dash.block")
                && BlockSpell.IsReady() && sender.IsValidTarget(Range))
            {
                foreach (var d in Spells.Where(x=> x.championname == sender.CharacterName
                && Initializer.Config["dash." + args.SData.Name].GetValue<MenuBool>().Enabled && 
                Initializer.Config["dash." + args.SData.Name] != null))
                {
