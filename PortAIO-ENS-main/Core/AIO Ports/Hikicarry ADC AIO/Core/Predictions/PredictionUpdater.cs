...
namespace HikiCarry.Core.Predictions
{
    public static class PredictionUpdater
    {
        ...
        {
            var objects = Geometry.ProjectOn(point,segmentStart, segmentEnd);
        ...
        public static Vector2[] CircleCircleIntersection(this Vector2 center1, Vector2 center2, float radius1, float radius2)
        {
            ...
            var direction = Geometry.Normalized((center2 - center1));
            var pa = center1 + (a * direction);
            var s1 = pa + (h * Geometry.Perpendicular(direction));
            var s2 = pa - (h * Geometry.Perpendicular(direction));
        ...
        {
            return Geometry.Distance(ObjectManager.Player,source);
        }
        ...
        {
            return Geometry.Distance(ObjectManager.Player,position);
        }
    }
}