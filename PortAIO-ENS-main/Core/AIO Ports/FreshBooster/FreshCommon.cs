...
namespace FreshBooster
{
    ...
    class FreshCommon
    {
        ...
        public static HitChance Hitchance(string Type)
        {
            ...
            switch (_MainMenu[Type].GetValue<MenuSlider>().Value)
            {
                case 1:
                    result = HitChance.OutOfRange;
                    break;
                case 2:
                    result = HitChance.None;
                    break;
                case 3:
        ...
        public static AIBaseClient GetFindObj(string name, float range, Vector3 Pos)
        {
            var CusPos = Pos;
            if (ObjectManager.Player.Distance(CusPos) > range) CusPos = ObjectManager.Player.Position.Extend(Game.CursorPos, range);
            var GetObj = ObjectManager.Get<AIBaseClient>().FirstOrDefault(f => f.IsAlly && !f.IsMe && f.Position.Distance(ObjectManager.Player.Position) < range && f.Distance(CusPos) < 150);
            if (GetObj != null)
                return GetObj;
            return null;
        }
