...
namespace EzAIO.Extras
{
    static class Helps
    {
        ...
        {
            return GetBestEnemyHeroesInRange(float.MaxValue,DamageType.Physical);
        }
        public static List<AIHeroClient> GetBestEnemyHeroesInRange(float range, DamageType damage)
        {
            return TargetSelector.GetTargets(range, damage).ToList();
        }
    }
}