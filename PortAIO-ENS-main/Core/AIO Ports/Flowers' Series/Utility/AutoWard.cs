 ...
 using ItemData = EnsoulSharp.SDK.Items;
 ...
 namespace Flowers_ADC_Series.Utility
{
    ...
    internal class AutoWard : Logic //This Part From SFX Utility 
    {
        ...
        {
            var AutoWardMenu = Menu.Add(new Menu("Auto Ward", "Auto Ward"));
            {
                AutoWardMenu.Add(new MenuBool("AutoWardEnable", "Enabled", true).SetValue(true));
                AutoWardMenu.Add(new MenuBool("AutoBush", "When target In Bush", true).SetValue(true));
                AutoWardMenu.Add(new MenuBool("AutoPink", "Auto Pink Ward", true).SetValue(true));
                AutoWardMenu.Add(new MenuBool("OnlyCombo", "Only Combo Mode Active", true).SetValue(true));
            }
        ...
        private static bool IsActive
            =>
            Menu["AutoWardEnable"].GetValue<MenuBool>().Enabled &&
            ((Menu["OnlyCombo"].GetValue<MenuBool>().Enabled && Orbwalker.ActiveMode == OrbwalkerMode.Combo) ||
             !Menu["OnlyCombo"].GetValue<MenuBool>().Enabled);
        ...
        private static void OnCreate(GameObject sender, EventArgs args)
        {
            ...
            if (!Menu["AutoPink"].GetValue<MenuBool>().Enabled)
            {
                return;
            }
        ...
        private static void OnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs Args)
        {
            ...
            if (spell != null && Menu["AutoPink"].GetValue<MenuBool>().Enabled)
            {
                if (!spell.Custom)
                {
                ...
                var Vayne = GameObjects.EnemyHeroes.Find(heros => heros.CharacterName.Equals("Vayne"));

                if (Vayne != null &&
                    spell.Hero.Equals(Vayne.CharacterName, StringComparison.OrdinalIgnoreCase))
                {
                    var buff =
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (!Menu["AutoBush"].GetValue<MenuBool>().Enabled)
            {
                return;
            }
        ...
        private static Vector3 GetWardPos(Vector3 lastPos, int radius = 165, int precision = 3)
        {
            ...
            while (count > 0)
            {
                ...
                for (var i = 0; i < vertices; i++)
                {
                    ...
                    wardLocations[i] = new WardLocation(pos, NavMesh.IsWallOfType(pos,CollisionFlags.Grass, 10));
                }
        ...
        private static SpellSlot GetRevealSlot(bool InBush = false)
        {
            if (!InBush)
            {
                if (Items.HasItem(Me,ItemId.Stealth_Ward) && ItemData.GetWardSlot(Me).SpellSlot.IsReady())
                ...
                if (Name == null && Slot != SpellSlot.Unknown)
                {
                    var champ =
                        GameObjects.EnemyHeroes.FirstOrDefault(
                            h => h.CharacterName.Equals(Hero, StringComparison.OrdinalIgnoreCase));
