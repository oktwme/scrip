...
namespace xSalice_Reworked.Managers
{
    ...
    public class SpellCastManager
    {
        ...
        public static void CastBasicSkillShot(Spell spell, float range, DamageType type,
            HitChance hitChance, bool towerCheck = false)
        {
            ...
            if (towerCheck && target.IsUnderEnemyTurret())
            {
                return;
            }
        ...
        public static void CastBasicFarm(Spell spell)
        {
            ...
            var minion = MinionManager.GetMinions(ObjectManager.Player.ServerPosition, spell.Range, MinionManager.MinionTypes.All, MinionManager.MinionTeam.NotAlly);
        ...
        public static void CastSingleLine(Spell spell, Spell spell2, bool wallCheck, float extraPrerange = 1)
        {
            ...
            var target = TargetSelector.GetTarget(spell.Range + spell2.Range, DamageType.Magical);
            ...
            var pred = Prediction.GetPrediction(target, spell.Delay);
            var rec1 = new Geometry.Geometry.Polygon.Rectangle(vector1, vector1.Extend(pred.CastPosition, spell2.Range), spell.Width);
            ...
            if (Player.ServerPosition.Distance(target.ServerPosition) < spell.Range)
            {
                ...
                var rec2 = new Geometry.Geometry.Polygon.Rectangle(vector2, vector2.Extend(pred.CastPosition, spell2.Range), spell.Width);
        ...
        public static void CastBestLine(bool forceUlt, Spell spell, Spell spell2, int midPointRange, Menu menu, float extraPrerange = 1, bool wallCheck = true)
        {
            ...
            foreach (var target in HeroManager.Enemies.Where(x => x.IsValidTarget(spell.Range)))
            {
                        ...
                        .OrderByDescending(x => x.Distance(target2.Position)))
                {
                    ...
                    var rec1 = new Geometry.Geometry.Polygon.Rectangle(startpos, endPos, spell.Width);

                    if (!rec1.Points.Exists(Util.IsWall) && Player.CountEnemyHeroesInRange(spell.Range + spell2.Range) > 2)
                    {
                        var target3 = target;
                        var enemy1 = enemy;
                        foreach (
                                    ...
                                    x.NetworkId != enemy1.NetworkId && x.Distance(target3.Position) < 1000))
                        {
                            ...
                            var obj = CommonPredEx.VectorPointProjectionOnLineSegment(startpos.To2D(), endPos.To2D(), enemy2Pred.CastPosition.To2D());
            ...
            if (start != Vector3.Zero && end != Vector3.Zero && spell.IsReady())
            {
                ...
                if (Orbwalker.ActiveMode == OrbwalkerMode.Combo &&
                    maxHit >= menu["Line_If_Enemy_Count_Combo"].GetValue<MenuSlider>().Value)
                {
                    CastLineSpell(start, end);
                }

                if (maxHit >= menu["Line_If_Enemy_Count"].GetValue<MenuSlider>().Value)
                {
                    CastLineSpell(start, end);
                }
            }
        ...
        public static void DrawBestLine(Spell spell, Spell spell2, int midPointRange, float extraPrerange = 1, bool wallCheck = true)
        {
            ...
            if (maxHit >= 2)
            {
                ...
                var rec1 = new Geometry.Geometry.Polygon.Rectangle(start, end, spell.Width);
            ...
            else
            {
                ...
                if (Player.ServerPosition.Distance(target.ServerPosition) < spell.Range)
                {
                    ...
                    var rec2 = new Geometry.Geometry.Polygon.Rectangle(vector1, vector1.Extend(pred.CastPosition, spell2.Range), spell.Width);
