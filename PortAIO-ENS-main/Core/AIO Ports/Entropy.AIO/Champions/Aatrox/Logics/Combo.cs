...
using SharpDX;
using Extensions = PortAIO.Library_Ports.Entropy.Lib.Constants.Extensions;
...
namespace Entropy.AIO.Aatrox.Logics
{
    ...
    static class Combo
    {
        ...
        {
            if (!ComboMenu.EBool.Enabled || !E.IsReady() || Q.IsReady() || Environment.TickCount - Q.LastCastAttemptTime <= 1200)
            {
                return;
            }
        ...
        {
            if (!ComboMenu.EBool.Enabled || !ComboMenu.EQBool.Enabled || !E.IsReady() || Environment.TickCount - Q.LastCastAttemptTime > 450)
            {
                return;
            }
        ...
        {
            if (!ComboMenu.EBool.Enabled || !E.IsReady() || Environment.TickCount - Q.LastCastAttemptTime > 450)
            {
                return;
            }
            ...
            if (!(GameObjectExtensions.DistanceToPlayer(target) > LocalPlayer.GetRealAutoAttackRange(target)) &&
