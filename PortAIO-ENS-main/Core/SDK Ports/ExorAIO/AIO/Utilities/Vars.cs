...
namespace ExorAIO.Utilities
{
    ...
    internal class Vars
    {
        ...
        public static Menu Menu { get; set; } = new Menu(
            $"aio.{GameObjects.Player.CharacterName.ToLower()}",
            $"[ExorAIO]: {GameObjects.Player.CharacterName}",
            true);
        ...
        /// </returns>
        public static float GetRealHealth(AIBaseClient target)
        {
            var debuffer = 0f;
            ...
            if (hero != null)
            {
                if (hero.CharacterName.Equals("Blitzcrank") && !hero.HasBuff("BlitzcrankManaBarrierCD"))
                {
