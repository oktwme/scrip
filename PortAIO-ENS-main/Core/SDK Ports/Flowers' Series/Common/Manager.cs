...
namespace Flowers_Series.Common
{
    public class Manager
    {
        ...
        /// <returns></returns>
        public static List<AIMinionClient> GetMinions(Vector3 From, float Range)
        {
        ...
        /// <returns></returns>
        public static List<AIMinionClient> GetMobs(Vector3 From, float Range, bool OnlyBig = false)
        {
            if (OnlyBig)
            {
        ...
        public static bool InAutoAttackRange(AttackableUnit target)
        {
            ...
            if (baseTarget != null)
            {
                return baseTarget.IsHPBarRendered &&
                    Vector2.DistanceSquared(baseTarget.Position.ToVector2(),
                    GameObjects.Player.Position.ToVector2()) <= myRange * myRange;
            }
        ...
        /// <param name="Target">Target</param>
        /// <returns></returns>
        public static float GetAttackRange(AIBaseClient Target)
        {
            if (Target != null)
            {
            ...
            bool CalCulateEDamage = true, bool CalCulateRDamage = true)
        {
            if (CheckTarget(target))
            {
                ...
                if (CalCulateQDamage)
                {
                    if (GameObjects.Player.CharacterName == "Ahri")
                    {
                    ...
                    else if (GameObjects.Player.CharacterName == "Viktor")
                    {
                        Damage += GameObjects.Player.HasBuff("ViktorPowerTransferReturn") ? GetQAttackDamage(target) : 0d;
                    }
                    else if (GameObjects.Player.CharacterName == "Vladimir")
                    {
        ...
        public static bool InHarass
        {
            get
            {
                return Orbwalker.ActiveMode == OrbwalkerMode.Harass;
            }
        }
        ...
        public static bool InLastHit
        {
            get
            {
                return Orbwalker.ActiveMode == OrbwalkerMode.LastHit;
            }
        }
        ...
        public static void WriteConsole(string Message, bool Chat = false)
        {
            ...
            if (Chat)
            {
                Game.Print("Flowers' Series : " + Message);
            }
        }
    }
}