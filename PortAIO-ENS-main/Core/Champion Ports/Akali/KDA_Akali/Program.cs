...
namespace KDA_Akali
{
    class Program
    {
        ...
        public static void OnLoad()
        {
            ...
            _q.SetSkillshot(0.25f, 70f, 1200f, false, SpellType.Cone);
            _e.SetSkillshot(0.25f, 70f, 1200f, true, SpellType.Line);
            _r.SetSkillshot(0.25f, 0f, float.MaxValue, false, SpellType.Line);
        ...
        private static void DoCombo()
        {
            ...
            var etarget = TargetSelector.GetTarget(2000f,DamageType.Physical);
            if (etarget != null)
            {
            ...
            switch (combomode.Index)
            {
                case 0:
                    ...
                    {
                        if (R2kill.Enabled && _r.GetDamage(t) >= t.Health || ObjectManager.Player.Buffs.Find(buff => buff.Name == "AkaliR").EndTime <= 500f && R2.Enabled)
                        {
                            _r.Cast(t);
                        }
                    }
        ...
        private static void DoHarass()
        {
            ...
            var etarget = TargetSelector.GetTarget(2000f,DamageType.Magical);
            if (t == null)
                return;
        ...
        private static float ComboFull(AIHeroClient t)
        {
            ...
            if(t != null)
            {
                ...
                if (_r.IsReady()) d = d + _r.GetDamage(t);
                if (_r.IsReady()) d = d + _r.GetDamage(t);
            ...
            { 
                ...
                if (t != null)
                {
                    if (_q.IsReady() && t.Health < _q.GetDamage(t) && t.IsValidTarget(_q.Range) && Qks.Enabled)
                        _q.Cast(t);
                    if ( _r.IsReady() && !ObjectManager.Player.HasBuff("AkaliR") && _r.GetDamage(t) > t.Health && R1ks.Enabled)
                    {
                        _r.Cast(t);
                    }
                    if (_r.IsReady() && ObjectManager.Player.HasBuff("AkaliR") && _r.GetDamage(t) > t.Health && R2ks.Enabled)
                    {
                        _r.Cast(t);
                    }
                }
