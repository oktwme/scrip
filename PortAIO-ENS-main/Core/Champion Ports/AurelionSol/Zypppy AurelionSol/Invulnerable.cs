...
namespace Zypppy_AurelionSol
{
    ...
    public class Invulnerable
    {
        ...
        static Invulnerable()
        {
            PEntries.AddRange(
                new List<InvulnerableEntry>
                    {
                        new InvulnerableEntry("UndyingRage") { ChampionName = "Tryndamere", MinHealthPercent = 1, CheckFunction = (target, type) => ((AIHeroClient)target).HealthPercent <= 1 },
                        ...
                        new InvulnerableEntry("KindredrNoDeathBuff") { MinHealthPercent = 10, CheckFunction = (target, type) => ((AIHeroClient)target).HealthPercent <= 10 }
                    });
        }
            ...
            float damage = -1f)
        {
            ...
            {
                if (entry.ChampionName == null || entry.ChampionName == hero.CharacterName)
                {
