...
namespace BadaoKingdom.BadaoChampion.BadaoGangplank
{
    public static class BadaoGangplankConfig
    {
        ...
        public static void BadaoActivate()
        {
            ...
            // main menu
            config = new Menu(ObjectManager.Player.CharacterName,"BadaoKingdom " + ObjectManager.Player.CharacterName, true);
            config.SetFontColor(SharpDX.Color.YellowGreen);
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                CircleRender.Draw(ObjectManager.Player.Position, BadaoMainVariables.Q.Range, Color.Yellow);
            }
            if (BadaoGangplankVariables.DrawE.GetValue<MenuBool>().Enabled)
            {
                CircleRender.Draw(ObjectManager.Player.Position, BadaoMainVariables.E.Range, Color.Pink);
            }
            if (BadaoGangplankVariables.DrawEPlacement.GetValue<MenuBool>().Enabled)
            {
                foreach (var barrel in BadaoGangplankBarrels.Barrels)
                {
                    CircleRender.Draw(barrel.Bottle.Position, 660, Color.Red );
                }
            }
        }
    }
