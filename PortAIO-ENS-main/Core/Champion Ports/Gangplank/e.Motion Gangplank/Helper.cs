...
namespace e.Motion_Gangplank
{
     public static class Helper
    {
        ...
        {
            return (int)(Player.Distance(position) / 2.6f + QDELAY + Game.Ping/2);
        }
        public static bool GetPredPos(AIHeroClient enemy, bool additionalReactionTimes = false, bool additionalBarrelTime = false)
        {
            //PredPos =  SPredictionMash1.Prediction.GetFastUnitPosition(enemy, enemyReactionTime.Value);
            float reactionDistance = enemyReactionTime.Value +
                                     ( additionalReactionTimes
                                         ? additionalReactionTime.Value
        ...
        {
            GetPredPos(enemy);
            return enemy.Position.Distance(kegPosition) <= 325 && PredPos.Distance(kegPosition) < 325 - enemy.MoveSpeed * Player.AttackCastDelay * 0.8f;
        }
        public static bool CannotEscape(this Vector3 kegPosition, AIHeroClient enemy, bool additionalReactionTimes = false, bool additionalBarrelTime = false)
        {
            GetPredPos(enemy, additionalReactionTimes, additionalBarrelTime);
            float fac = 0.0008f * enemy.MoveSpeed * (GetQTime(kegPosition) + (additionalBarrelTime ? 400 : 0) -
                                                     (additionalReactionTimes ? additionalReactionTime.Value : 0) -
                                                     enemyReactionTime.Value);
