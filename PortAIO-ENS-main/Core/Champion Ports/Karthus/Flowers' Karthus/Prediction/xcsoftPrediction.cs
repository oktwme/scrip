...
namespace Flowers_Karthus.Prediction
{
    ...
    {
        public static float PredHealth(AIBaseClient Target, Spell spell) => SebbyLib.HealthPrediction.GetHealthPrediction(Target, (int)(ObjectManager.Player.Distance(Target) / spell.Speed), (int)(spell.Delay * 1000 + Game.Ping / 2));

        public static void CastCircle(this Spell spell, AIBaseClient target)
        {
            if (spell.Type == (SpellType) SkillshotType.SkillshotCircle || spell.Type == (SpellType) SkillshotType.SkillshotCone)
            {
                if (target == null)
                {
                    return;
                ...
                var pred = SebbyLib.Prediction.GetPrediction(target, spell.Delay, spell.Width / 2, spell.Speed);
                ...
                if (target.IsValidTarget(spell.Range))
                {
                    if (
                        !(target.MoveSpeed*
                          ...
                          spell.Width*1/2))
                    {
                        if (pred.Hitchance < (SebbyLib.HitChance) HitChance.VeryHigh ||
        ...
        public static void CastLine(this Spell spell, AIBaseClient target, float alpha = 0f, float colmini = float.MaxValue, bool HeroOnly = false, float BombRadius = 0f)
        {
            if (spell.Type != (SpellType) SkillshotType.SkillshotLine)
            {
                return;
            }
        ...
        public static void CastCone(this Spell spell, AIBaseClient target, float alpha = 0f, float colmini = float.MaxValue, bool HeroOnly = false)
        {
            if (spell.Type != (SpellType) SkillshotType.SkillshotCone)
            {
                return;
            }
            ...
            if (target.IsValidTarget(spell.Range - target.MoveSpeed * (spell.Delay + ObjectManager.Player.Distance(target.ServerPosition) / spell.Speed) + alpha) && minioncol <= colmini && pred.Hitchance >= (SebbyLib.HitChance) HitChance.VeryHigh)
            {
                spell.Cast(pred.CastPosition);
            }
        }

        public static void CastAOE(this Spell spell, AIBaseClient target)
        {
            ...
            var pred = SebbyLib.Prediction.GetPrediction(target, spell.Delay > 0 ? spell.Delay : 0.25f, spell.Range);

            if (pred.Hitchance >= (SebbyLib.HitChance) HitChance.High && pred.UnitPosition.Distance(ObjectManager.Player.ServerPosition) <= spell.Range)
            {
                spell.Cast();
            }
        }
        ...
        public static void RTarget(this Spell spell, AIBaseClient Target)
        {
        ...
        public static void NTarget(this Spell spell, AIBaseClient Target)
        {
        ...
        public static bool CanHit(this Spell spell, AIBaseClient T, float Drag = 0f)
        {
