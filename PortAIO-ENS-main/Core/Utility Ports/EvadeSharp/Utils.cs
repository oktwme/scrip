...
namespace Evade
{
    public static class Utils
    {
        ...
        public static Vector3[] GetPath2(this AIHeroClient hero, Vector3 end)
        {
        ...
        public static void SendMovePacket(this AIBaseClient v, Vector2 point, bool triggerEvents = false)
        {
            if (ObjectManager.Player.CanMove)
                ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, LeagueSharpCommon.Geometry.Geometry.To3D(point), triggerEvents);
        }
            ...
            foreach (var buff in unit.Buffs)
            {
                if (buff.IsActive && Game.Time <= buff.EndTime &&
                    (buff.Type == BuffType.Charm || buff.Type == BuffType.Knockup || buff.Type == BuffType.Knockback || buff.Type == BuffType.Stun ||
                     buff.Type == BuffType.Suppression || buff.Type == BuffType.Snare || buff.Type == BuffType.Asleep))
                {
