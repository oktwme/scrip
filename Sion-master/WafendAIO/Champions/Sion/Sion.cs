...
namespace WafendAIO.Champions
{
    public class Sion
    {
        ...
        public static void initializeSion()
        {
            ...
            menucombo.Add(new MenuBool("useQ", "Use Q", true));
            menucombo.Add(new MenuBool("useW", "Use W", true));
            menucombo.Add(new MenuBool("useE", "Use E", true));
            ...
            var menuExploits = new Menu("exploitSettings", "Exploit")
            {
                ...
                new MenuKeyBind("RKey", "R Key", Keys.T, KeyBindType.Press),
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                Drawing.DrawCircleIndicator(Game.CursorPos, Config["drawingSettings"].GetValue<MenuSlider>("champRadius").Value,
                    Color.Red);
            }

            if (Config["drawingSettings"].GetValue<MenuBool>("drawMinionRadius").Enabled)
            {
                Drawing.DrawCircleIndicator(Game.CursorPos, Config["drawingSettings"].GetValue<MenuSlider>("minionRadius").Value,
                    Color.Blue);
            }
            ...
            {
                Drawing.DrawCircleIndicator(ObjectManager.Player.Position, Q.ChargedMaxRange, Color.Pink);
            }

            if (Config["drawingSettings"].GetValue<MenuBool>("drawERange").Enabled)
            {
                Drawing.DrawCircleIndicator(ObjectManager.Player.Position, E.Range, Color.Red);
            }
            ...
            if (posVec != null)
            {
                Drawing.DrawCircleIndicator(posVec, 50, Color.Blue);
            }
        }
        ...
        private static void Combo()
        {
            ...
            if (Config["comboSettings"].GetValue<MenuBool>("useE").Enabled)
            {
            ...
            if (Config["comboSettings"].GetValue<MenuBool>("useQ").Enabled)
            {
            ...
            if (Config["comboSetiings"].GetValue<MenuBool>("Wuse").Enabled)
            {
                var wtarget = TargetSelector.GetTarget(525, DamageType.Magical);
