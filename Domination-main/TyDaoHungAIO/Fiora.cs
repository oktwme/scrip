...
namespace DaoHungAIO.Champions
{
...
)
        {
            ...
            foreach (var spellData in spellDatas)
            {
                ...
                if (spellData.CharacterName == "Azir" && spellData.Slot == SpellSlot.R && args.Slot == SpellSlot.R)// chua test
                {
                    ...
                    Drawing.DrawCircleIndicator(start.ToVector3(), 50, System.Drawing.Color.Red);
                    Drawing.DrawCircleIndicator(end.ToVector3(), 50, System.Drawing.Color.Red);
    ...
    EventArgs args
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            if (Player.IsDead)
                return;
            if (DrawQ)
                Drawing.DrawCircleIndicator(Player.Position, 400, Color.Green);
            if (DrawW)
            {
                Drawing.DrawCircleIndicator(Player.Position, W.Range, Color.Green);
            }
            if (DrawOptionalRange && TargetingMode == GetTargets.TargetMode.Optional)
            {
                Drawing.DrawCircleIndicator(Player.Position, OptionalRange, Color.DeepPink);
            }
            if (DrawSelectedRange && TargetingMode == GetTargets.TargetMode.Selected)
            {
                Drawing.DrawCircleIndicator(Player.Position, SelectedRange, Color.DeepPink);
            }
            if (DrawPriorityRange && TargetingMode == GetTargets.TargetMode.Priority)
            {
                Drawing.DrawCircleIndicator(Player.Position, PriorityRange, Color.DeepPink);
            }
            if (DrawTarget && TargetingMode != GetTargets.TargetMode.Normal)
            {
                var hero = GetTarget();
                if (hero != null)
                    Drawing.DrawCircle(hero.Position, 75, 5, System.Drawing.Color.Yellow);
            }
            if (DrawVitals && TargetingMode != GetTargets.TargetMode.Normal)
            {
                ...
                if (hero != null)
                {
                    ...
                    if (status.HasPassive && status.PassivePredictedPositions.Any())
                    {
                        ...
                        {
                            Drawing.DrawCircleIndicator(x.ToVector3(), 50, Color.Yellow);
                        }
                    }
                }
            }
                    ...
                    if (Lstwall != null)
                    {
                        ...
                        if (InMiddileWall(firstwall, lastwall))
                        {
                            for (int i = 0; i <= 359; i++)
                            {
                                ...
                                {
                                    Drawing.DrawCircleIndicator(firstwall.ToVector3(), 50, Color.Green);
                                    goto Finish;
                                }
                            }

                            Drawing.DrawCircleIndicator(firstwall.ToVector3(), 50, Color.Red);
                        }
                    }
                }
                Finish:;
    ...
    class HINH1
    {
        ...
        {
            Drawing.DrawCircle(center, radius, width, color);
            return;
            var pos1y = center;
        ...
        public static void DrawCircle(Vector3 center,
            ...
            int quality = 60)
        {
            Drawing.DrawCircle(center, radius, thickness, color);
